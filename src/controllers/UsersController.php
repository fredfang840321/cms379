...
class UsersController extends Controller
{
    ...
    protected $allowAnonymous = [
        ...
        'loginbyphone' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
        'bindphone' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
        ...
        'send-password-reset-phone' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
        'send-password-reset-finduser' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
        'set-password' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
        // 'set-password-byphone' => self::ALLOW_ANONYMOUS_LIVE | self::ALLOW_ANONYMOUS_OFFLINE,
    ...
    public function actionLogin()
    {
        ...
        Craft::$app->session->setFlash('loginName',$loginName);
        if (!$user || $user->password === null) {
            ...
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Invalid username or password.'));
        }

        // var_dump($user->phone);die;
        ...
        $redirect = $this->request->getBodyParam('redirect');
        // 如果没有绑定手机
        if(!$user->phone && $redirect){
            // 跳转到绑定手机号

            $head = $this->getCurUrl();
            $url = $head.$_SERVER['SERVER_NAME'].'/zh/registration/?binding-number=true';

            Craft::$app->session->setFlash('email',$loginName);
            if(!strstr($redirect, 'checkout/shipping/address')){
                Craft::$app->session->setFlash('newredirect',$head.$_SERVER['SERVER_NAME'].'/zh/myaccount/overview');
            }else{
                Craft::$app->session->setFlash('newredirect',$head.$_SERVER['SERVER_NAME'].'/zh/checkout/shipping/address');
            }
    ...
    public function actionLoginbyphone()
    {
        ...
        $phone = $this->request->getRequiredBodyParam('phone');
        $code = $this->request->getRequiredBodyParam('code');
        Craft::$app->session->setFlash('loginphone',$phone);
        Craft::$app->session->setFlash('logincode',$code);
        $cpcode = $this->request->getRequiredBodyParam('codetruesignin');
        // var_dump($cpcode);die;
        if($cpcode!=1){
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Please enter correct verification code.'));
        }

        $rememberMe = (bool)$this->request->getBodyParam('rememberMe');
        // Does a user exist with that username/email?
        $user = Craft::$app->getUsers()->getUserByPhone($phone);
        if (!$user) {
            // Delay again to match $user->authenticate()'s delay
            Craft::$app->getSecurity()->validatePassword('p@ss1w0rd', '$2y$13$nj9aiBeb7RfEfYP3Cum6Revyu14QelGGxwcnFUKXIrQUitSodEPRi');
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Mobile phone hasn’t been registered, please register.'));
        }

        $codeinfo = Craft::$app->db->createCommand('SELECT * FROM craft_chinacms WHERE phone='.$phone)
           ->queryOne();
        if(empty($codeinfo) || $codeinfo['code']!=$code){
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Please enter correct SMS code.'));
        }
        if(isset($codeinfo['update_time']) && (time()-$codeinfo['update_time'])>300){
            return Craft::$app->session->setFlash('error',Craft::t('app', 'SMS code is expired, please request again.'));
        }
    ...
    public function actionBindphone()
    {
        ...
        Craft::$app->session->setFlash('email',$email);
        ...
        if (!$user) {
            // Delay again to match $user->authenticate()'s delay
            Craft::$app->getSecurity()->validatePassword('p@ss1w0rd', '$2y$13$nj9aiBeb7RfEfYP3Cum6Revyu14QelGGxwcnFUKXIrQUitSodEPRi');
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Email address hasn’t been registered, please register.'));
        }

        $user2 = Craft::$app->getUsers()->getUserByPhone($phone);
        if ($user2) {
            return Craft::$app->session->setFlash('error',Craft::t('app', 'Email address hasn’t been registered, please register.'));
        }
        ...
        // 绑定手机号 
        $codeinfo = Craft::$app->db->createCommand()->update('craft_users', ['phone' => $phone], "username='{$email}'")->execute();
        ...
        // Get the return URL
        $userSession = Craft::$app->getUser();
        $returnUrl = $this->request->getRequiredBodyParam('redirect');
    ...
    public function actionSendPasswordResetEmail()
    {
        ...
        // $redirct = $this->request->getBodyParam('redirct');
        Craft::$app->session->setFlash('forgotname',$loginName);

        // 判断是否为手机号
        if(is_numeric($loginName)){
            if(preg_match("/^1[3456789]\d{9}$/", $loginName)){
                return $this->SendPasswordResetPhone();
            }else{
                return $this->setFailFlash(Craft::t('app', 'Please enter correct phone number or email address.'));
            }
        }else{
        ...
        if (!isset($user)) {
            ...
            if (!$user) {
                return $this->setFailFlash(Craft::t('app', 'Email address hasn’t been registered, please register.'));
            }
        }
    ...
    // 手机号找回密码
    public function SendPasswordResetPhone()
    {
        $this->requirePostRequest();
        $errors = [];
        $existingUser = false;
        ...
        $loginName = $this->request->getBodyParam('loginName');
        $redirct = $this->request->getBodyParam('redirct');
        // var_dump($redirct);die;
        if (!$loginName) {
            // return $this->_handleSendPasswordResetError($errors);
            return $this->setFailFlash(Craft::t('app', 'Please enter correct phone number or email address.'));
        }

        $user = Craft::$app->getUsers()->getUserByPhone($loginName);

        if (!$user) {
            return $this->setFailFlash(Craft::t('app', 'Mobile phone hasn’t been registered, please register.'));
        }

        Craft::$app->session->setFlash('phone',$loginName);
    ...
    /**
     * Sends a password reset phone.
     *
     * @return Response|null
     * @throws NotFoundHttpException if the requested user cannot be found
     */
    public function actionSendPasswordResetFinduser()
    {
        $this->requirePostRequest();
        $code = $this->request->getBodyParam('code');
        $phone = $this->request->getBodyParam('phone');

        Craft::$app->session->setFlash('phone',$phone);
        if (!$phone) {
            // return $this->_handleSendPasswordResetError($errors);
            return $this->setFailFlash('Please wait');
        }
        ...
        $codeinfo = Craft::$app->db->createCommand('SELECT * FROM craft_chinacms WHERE phone='.$phone)
           ->queryOne();
        if(empty($codeinfo) || $codeinfo['code']!=$code){
            return $this->setFailFlash(Craft::t('app', 'Please enter correct SMS code.'));
        }
        if(isset($codeinfo['update_time']) && (time()-$codeinfo['update_time'])>300){
            return $this->setFailFlash(Craft::t('app', 'SMS code is expired, please request again.'));
        }
     ...
     * @throws NotFoundHttpException if the requested user cannot be found
     */
    public function setPasswordByphone()
    {
        $this->requirePostRequest();
        ...
        // var_dump($this->redirectToPostedUrl());die;
        return $this->redirectToPostedUrl();
    ...
    public function actionSetPassword(): Response
    {
        ...
        $uid = $this->request->getRequiredParam('id');
        if(!$uid){
            return $this->setPasswordByphone();
        }
    ...
    public function actionSaveUser()
    {
        ...
        if ($isPublicRegistration) {
            ...
            if ($isCurrentUser) {
                ...
                $changepass = $this->request->getBodyParam('changepass');
                if($changepass){
                    if(!$newpass){
                        $user->addError('currentPassword', Craft::t('app', 'Please enter a new password.'));
                        Craft::$app->session->setFlash('errorPass',Craft::t('app', 'Please enter a new password.'));
                    }else{
                        if($user->authenticate($newpass)){
                            $user->addError('currentPassword', Craft::t('app', 'The new password cannot be the same as the old password.'));
                            Craft::$app->session->setFlash('errorPass',Craft::t('app', 'The new password cannot be the same as the old password.'));
                        }
                    }
                }
            }
        ...
        if (
            ...
            Craft::$app->session->setFlash('errorPass',Craft::t('app', 'Incorrect current password.'));
        }
        ...
        if (
            ...
            if(isset($user->errors['email']) && !empty($user->errors['email'])){
                // Craft::$app->session->setFlash('emailerror',['邮箱已被使用']);
                Craft::$app->session->setFlash('emailerror',[Craft::t('app', 'Mailbox is already in use.')]);
            }
            if(isset($user->errors['username']) && !empty($user->errors['username'])){
                Craft::$app->session->setFlash('emailerror',[Craft::t('app', 'Mailbox is already in use.')]);
            }
        ...
        if ($phone && $isNewUser) {
            $head = $this->getCurUrl();
            $url= $head.$_SERVER['SERVER_NAME'].'/china-kaimou/sendsms?phone='.$phone.'&type=2';
